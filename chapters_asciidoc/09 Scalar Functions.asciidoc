...
Negative numbers can be used in bit manipulation. For example the SMALLINT value -1 will have all 16 bits set to "1" (see example on page: 131). As their name implies, the BIT functions can be used to do bit-by-bit comparisons between two numbers:
...
Returns null if the two values being compared are equal, otherwise
